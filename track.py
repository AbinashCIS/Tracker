...
counter = 0
features = {'changes': 0, 'counter': 0, 'identities': []}
...
def write_csv(path):
	global features
	txt_path = str(Path(path)) + '/results.csv'
	with open(txt_path, 'a') as f:
		f.write(
		    f"{datetime.now()},{features['changes'] if features['changes'] != features['counter'] else 0},{features['counter']},{len(features['identities'])},\n"
		)  # label format
